Ext.define('DelUserPanel', {
	...
	initComponent: function() {
		this.store = Ext.create('Ext.data.Store', {
            ...
            proxy: {
                type: 'ajax',
                url: 'control/admin/user.do?method=listdel',  //请求的服务器地址
                reader: {
                    type: 'json',
                    root: 'records',
                    totalProperty: 'count'
            ...
            {header: '操作', dataIndex: 'userid',
            	renderer: function(value) {
            		return '<div align="center">' +
            				'<a href="javascript:resetUser(' + value + ');">重新启用</a>' +
            				'</div>';
            	}
            }
        ];
...
function resetUser(id) {
	...
	Ext.Ajax.request({
		url: 'control/admin/check/deluser.do',
		async: false,
		method: 'POST',
		success: function(response, opt) {
			...
			if(!r.deluser) return;
				...
				Ext.MessageBox.confirm('提示', '确定要重新启用该用户？', 
					function(btn) {
						if(btn == 'yes') {
							Ext.Ajax.request({
									url: 'control/admin/user.do?method=reset',
									params: {id: id},
									method: 'POST',
									success: function(response, opt) {
										var res = Ext.JSON.decode(response.responseText);
										if(res.success == true)
											Ext.getCmp('dupanel').getStore().reload();
										else {
											Ext.MessageBox.show({
												title: '提示',
												msg: '重新启用操作失败！',
