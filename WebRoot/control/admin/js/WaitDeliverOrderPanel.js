Ext.define('WaitDeliverOrderPanel', {
	...
	initComponent: function() {
		this.store = Ext.create('Ext.data.Store', {
            ...
            proxy: {
                type: 'ajax',
                url: 'control/admin/order.do?method=list&os=4',  //请求的服务器地址
                reader: {
                    type: 'json',
                    root: 'records',
                    totalProperty: 'count'
	...
	onLoadOrder: function() {
		var win;
		var sm = Ext.getCmp('wdopanel').getSelectionModel();
			...
			win = new Ext.Window({
				...
				buttons: [{
					...
					handler: function() {
						...
						Ext.getCmp('wdopanel').getStore().reload();
				...
				listeners: {
					afterrender: {
		        		fn: function() {
		        			Ext.Ajax.request({
								url: 'control/admin/check/deliver.do',
								async: false,
								method: 'POST',
								success: function(response, opt) {
									var r = Ext.JSON.decode(response.responseText);
									Ext.getCmp('mdop').disabled = !r.deliver;
									Ext.getCmp('mdolm').disabled = !r.leafmsg;
								}
							});
		        		}
		        	}
