		...
		<tbody><tr>
			...
			<c:if test="${totalPage > 1}">
				<c:choose>
					...
					<c:otherwise>
						<td><a href="javascript:void(0)" onclick="showtypetjlist(${pre}, ${limit})">上一页</a></td>
					</c:otherwise>
				</c:choose>
				<c:forEach begin="1" end="${totalPage}" varStatus="v">
					<c:choose>
						...
						<c:otherwise>
							<td><a href="javascript:void(0)" onclick="showtypetjlist(${(v.count - 1) * limit}, ${limit})">${v.count}</a></td>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				<c:choose>
					...
					<c:otherwise>
						<td><a href="javascript:void(0)" onclick="showtypetjlist(${next}, ${limit})">下一页</a></td>
					</c:otherwise>
				</c:choose>
			</c:if>
			</tr></tbody>
